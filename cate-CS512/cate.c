...
int kappaRankingComparator(const void *a, const void *b) { // small -> large
  int aSmall = kappa[*(int *) a] < kappa[topic_pivot_idx];
  int bSmall = kappa[*(int *) b] < kappa[topic_pivot_idx];
  if (aSmall == bSmall) {
...
int productRankingComparator(const void *a, const void *b) { // small -> large
  long long rankA = 1LL * kappa_rankings[(*(int *) a) % vocab_size] * sim_rankings[*(int *) a];
  long long rankB = 1LL * kappa_rankings[(*(int *) b) % vocab_size] * sim_rankings[*(int *) b];
  if (sim_rankings[*(int *) a] > similaritySearchSize || kappa[(*(int *) a) % vocab_size] < kappa[topic_pivot_idx]) {
    rankA = 1e18;
  }
  if (sim_rankings[*(int *) b] > similaritySearchSize || kappa[(*(int *) b) % vocab_size] < kappa[topic_pivot_idx]) {
    rankB = 1e18;
  }
...
void InitNet() {
  ...
  if (with_regularization) {
    ...
    if (rank_product) {
      ...
      a = posix_memalign((void **) &kappa_rankings, 128, (long long) vocab_size * sizeof(int));
      for (a = 0; a < vocab_size; a++) kappa_rankings[a] = a;
    }
  }
}
  ...
  if (with_regularization) {
    ...
    for (iter_count = pretrain_iters; iter_count < iter; iter_count++) {
      ...
      if (rank_product && multi_keywords == 2) {
        printf("Rank product\n");
        qsort(rankings, vocab_size, sizeof(int), kappaRankingComparator);
        for (b = 0; b < vocab_size; b++) kappa_rankings[rankings[b]] = b + 1;
        for (a = 0; a < topics; a++) {
      ...
      for (a = 0; a < topics; a++) {
        printf("Category: %ld (%s)\t", a, vocab[topic_index[topic_start_end[a * 2]]].word);
        for (b = 0; b < num_per_topic; b++) {
